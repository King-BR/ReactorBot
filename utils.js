...
module.exports = {
   ...
   */
	...
	//#endregion
	//--------------------------------------------------------------------------------------------------//
	//#region Discord Server utils

	/**
	 * @param {Discord.Client} client
	 * @param {String} winnerMemberID
	 * @param {Number} miniquizQuestion
	 * @param {Discord.Message?} winMessage
	 */

	closeMiniquiz: (client, winnerMemberID, miniquizQuestion, winMessage = null) => {

		if (winMessage) winMessage.react('✅');
		...
		// Configura o Canal
		miniquizChannel.send(`\`${client.users.cache.get(winnerMemberID).tag}\` respondeu certo! ganhou ${money}\$ e ${xp} xp`);
		miniquizChannel.overwritePermissions([
			{ id: '700183808783286372', allow: 805829713 },
			{ id: '755665930159390721', deny: 2112 },
			{ id: '699823229354639471', allow: 66624, deny: 805763089 }
		]);

		module.exports.jsonChange("./dataBank/serverState.json", serverState => {

			serverState.eventWin = null;
			serverState.eventType = null;
			serverState.miniquizChooses = {};

			return serverState;
		}, true)

		require('./database.js').Users.findById(winnerMemberID, (err, doc) => {
			if (err) {
				console.log(
					'\n=> ' +
			...
			if (!doc) doc = new Users({ _id: winnerMemberID });
